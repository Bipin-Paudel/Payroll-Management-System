...
import { api } from "@/lib/api"; // 🔁 change to "@/utils/api" if your api is there
...
function normalizeList(payload: any): RoleRow[] {
...
export default function RolesPage() {
  ...
  async function fetchRoles() {
    setLoading(true);
    try {
      // ✅ interceptor attaches token + refreshes automatically if token expired
      const res = await api.get("/roles");
      setRows(normalizeList(res.data));
    } catch (err) {
      setServerMsg({
        type: "error",
        text: toAxiosMessage(err, "Failed to load roles."),
      });
    } finally {
      setLoading(false);
    }
  ...
  // ✅ one perfect refresh function for everything
  async function reload() {
    await fetchRoles(); // client state refresh
    router.refresh();  // app router cache refresh (safe even if not needed)
  }
  ...
  function validateEdit() {
    ...
    if (!editName.trim()) e.name = "Role name is required.";
  ...
  async function saveEdit(id: string) {
    ...
    try {
      ...
      setServerMsg({ type: "success", text: "Role updated successfully." });
      cancelEdit();
      await reload();
    } catch (err) {
      setServerMsg({
        type: "error",
        text: toAxiosMessage(err, "Failed to update role."),
      });
    } finally {
      setSavingId(null);
    }
  ...
  async function deleteRole(id: string) {
    ...
    try {
      ...
      setServerMsg({ type: "success", text: "Role deleted successfully." });
      setConfirmDeleteId(null);
      ...
      setServerMsg({
        type: "error",
        text: toAxiosMessage(err, "Failed to delete role."),
      });
    } finally {
      setDeletingId(null);
    }
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            View, edit, and manage roles used across your payroll system.
          </p>
        </div>
        ...
        <div className="p-5 sm:p-6 md:p-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="text-sm text-muted-foreground">
              {loading ? "Loading..." : `${filtered.length} role(s)`}
            </div>
          </div>
          ...
          <div className="mt-6 overflow-x-auto rounded-xl border border-border bg-background">
            <table className="kp-table">
              <thead>
                <tr>
                  <th className="kp-th">Role Name</th>
                        ...
                        <td className="kp-td">
                          {isEditing ? (
                            <div>
                              ...
                              />
                              {fieldErrors.name && (
                                <p className="mt-2 ui-error">{fieldErrors.name}</p>
                              )}
                            </div>
                          ) : (
                        ...
                        <td className="kp-td">
                          {isEditing ? (
                            <div>
                              ...
                              />
                              {fieldErrors.description && (
                                <p className="mt-2 ui-error">{fieldErrors.description}</p>
                              )}
                            </div>
                          ) : (
      ...
      <Dialog open={openCreate} onOpenChange={setOpenCreate}>
        <DialogContent className="w-[94vw] sm:w-[60vw] max-w-none rounded-xl border bg-card shadow-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Add a new role for your payroll system.
            </DialogDescription>
          </DialogHeader>
