...
import { api } from "@/lib/api"; // 🔁 change to "@/utils/api" if needed
...
function normalizeList(payload: any): PaymentMethodRow[] {
...
export default function PaymentMethodsPage() {
  ...
  const [rows, setRows] = React.useState<PaymentMethodRow[]>([]);
  ...
  async function fetchPaymentMethods() {
    ...
    try {
      ...
      setServerMsg({
        type: "error",
        text: toErrorMessage(err, "Failed to load payment methods."),
      });
    } finally {
      setLoading(false);
    }
  ...
  // ✅ Perfect refresh (client + app router cache)
  ...
  function startEdit(r: PaymentMethodRow) {
    setServerMsg(null);
  ...
  function validateEdit() {
    ...
    if (!editName.trim()) e.name = "Payment method name is required.";
    else if (editName.trim().length < 2)
      e.name = "Payment method name must be at least 2 characters.";
  ...
  async function saveEdit(id: string) {
    ...
    try {
      ...
      setServerMsg({
        type: "error",
        text: toErrorMessage(err, "Failed to update payment method."),
      });
    } finally {
      setSavingId(null);
    }
  ...
  async function deletePaymentMethod(id: string) {
    ...
    try {
      ...
      setServerMsg({
        type: "error",
        text: toErrorMessage(err, "Failed to delete payment method."),
      });
    } finally {
      setDeletingId(null);
    }
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            View, edit, and manage payment methods used across your payroll
            system.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
          ...
          >
            {editMode ? "Done" : "Edit Payment Methods"}
          </button>
        ...
        <div className="p-5 sm:p-6 md:p-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div className="w-full sm:max-w-md">
              ...
              <div className="mt-2">
                <input
                  ...
                  placeholder="Search by payment method name or description..."
            ...
            <div className="text-sm text-muted-foreground">
              {loading ? "Loading..." : `${filtered.length} payment method(s)`}
            </div>
          </div>
          ...
          {/* ✅ SAME AS ROLES/DEPARTMENTS: KP table wrapper + zebra + hover */}
          <div className="mt-6 overflow-x-auto rounded-xl border border-border bg-background">
            <table className="kp-table">
              <thead>
                <tr>
                  <th className="kp-th">Payment Method Name</th>
                                ...
                                {!isConfirmingDelete ? (
                                  ...
                                  <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
                                    <button
                                      type="button"
                                      disabled={isDeleting}
                                      onClick={() => deletePaymentMethod(r.id)}
                                      className={cn(
                                        "h-11 w-full sm:w-auto rounded-md bg-destructive px-4 text-sm font-medium text-white shadow-xs",
                                        "transition-colors hover:opacity-90",
                                        "focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background",
      ...
      <Dialog open={openCreate} onOpenChange={setOpenCreate}>
        <DialogContent className="w-[94vw] sm:w-[60vw] max-w-none rounded-xl border bg-card shadow-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Add a new payment method for your payroll system.
            </DialogDescription>
          </DialogHeader>
